...
(async () => {
  ...
  // Most addresses end in .com, and wedata has general rules for any .com page.
  const generalOnly = 'https://shop.unlisted-example.com/list?page=2';
  ...
  const rows = [
    ...
    ['a .com address only general rules match, first use', `${(await cold(generalOnly)).toFixed(2)} ms`],
